...
public class ImprovedECStack_SmartWait<T> extends EliminationCombiningStack<T> implements ConcurrentStack<T> {
    ...
    public ImprovedECStack_SmartWait(int nThreads) {
        super(nThreads);
    }
    ...
    public boolean collide(MultiOp<T> mOp) {
        ...
        if (!location[ThreadID.get()].compareAndSet(mOp, null)) {
            adaptiveWaiter.get().increaseWait(); //TODO was increase
            return passiveCollide(mOp);
        }
        adaptiveWaiter.get().decreaseWait();
        return false;
    }
}
